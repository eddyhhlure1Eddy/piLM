"""Export a piLM quantized asset bundle (W8A32 or W4A16).

Reuses an already-built per-Linear quant cache (hardlinked into the bundle's
linear_<fmt> dir) plus streamed BF16 residual/mtp shards. Streaming is
shard-by-shard so peak RAM stays a few GB even for a 9B source -- this is what
avoids the Windows page-file "os error 1455" that blocks loading the 18 GB
checkpoint in one shot while the W4 live server is resident.

The manifest matches the v1 format the runtime loader already understands
(see models/qwen3/weights.py: _load_quant_bundle_manifest / _bundle_weight_map).
"""
...
# quantize label -> (linear subdir name, quant_format the loader expects)
FORMATS = {
    "w8a32-all": ("linear_w8a32", "w8a32"),
    "w4a16-all": ("linear_w4a16", "w4a16"),
}
...
def export_bundle(args: argparse.Namespace) -> dict:
    ...
    linear_subdir, quant_format = FORMATS[args.quantize]
    ...
    linear_dir = out_dir / linear_subdir
...
def main() -> None:
    ...
    parser.add_argument("--cache-dir", required=True,
                        help="dir holding the per-Linear {digest}-{leaf}.safetensors quant cache to hardlink")
    parser.add_argument("--quantize", choices=list(FORMATS), required=True)
    parser.add_argument("--include-mtp", action="store_true")
    args = parser.parse_args()
    manifest = export_bundle(args)
    summary = {k: manifest[k] for k in (
        "quantize", "quant_format", "linear_subdir",
        "quantized_linear_tensors", "residual_bf16_tensors", "mtp_bf16_tensors",
        "missing_linear_cache", "linear_cache_link_modes", "bundle_gib",
    )}
